                ...
                <div class="card-body"> 
                    <div class="row">
                        ...
                        <div class="col-8 text-right">
                            <button type="button" onclick="location.href='<?= base_url('master/Locket/create') ?>'" class="btn btn-sm btn-success"><i class="fa fa-plus"></i> Tambah Loket</button>
    ...
    function loadtable(status) {
        var table = '<table class="table table-bordered dt-responsive nowrap " style="border-collapse: collapse; border-spacing: 0; width: 100%;" id="mytable">'+
             ...
             $("#load-table").html(table)
              var t = $("#mytable").DataTable({
                ...
                processing: true,
                serverSide: true,
                ajax: {"url": "<?= base_url('master/Locket/json?status=') ?>"+status, "type": "POST"},
                columns: [
                ...
                columnDefs : [
                    ...
                    { 
                      targets : [4],
                        render : function (data, type, row, meta) {
                          let a = row['json_service']
                          a = JSON.parse(a);
                          let jsonReturn = a.map(item => { return item.name }).join(", ")
                          return jsonReturn;
                        }
                    },
         ...
         function edit(id,e) {
              location.href = "<?= base_url('master/Locket/edit/') ?>"+id;
         }   

      function create() {
        $("#load-form").html('loading...');
        $("#modal-form").modal();
        $("#title-form").html('Create Loket');
        $("#load-form").load("<?= base_url('master/Loket/create/') ?>");
      }
        ...
        Swal.fire({
          ...
          confirmButtonText: 'Yes, delete it!'
            ...
            if (result.isConfirmed) {
              $.ajax({
                  url: '<?= base_url('master/locket/delete/')?>',
                type: 'post',
                dataType: 'html',
                data: {id: id},
                beforeSend:function () { },
      ...
      function hapuspilihdata() {
        ...
        confirmButtonText: 'Yes, delete it!'
          ...
          if (result.isConfirmed) {
            $.ajax({
              url: '<?= base_url('master/locket/Deletedata/')?>',
              type: 'post',
              dataType: 'html',
              data: {data: data},
              beforeSend:function () {
